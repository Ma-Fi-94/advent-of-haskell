...
-- Pad a 2d array with a given element along all four sides
pad :: a -> [[a]] -> [[a]]
pad x ls = [hor] ++ midlines ++ [hor]
  where
    hor      = replicate (length (ls!!0) + 2) x
    midlines = map (\l -> [x] ++ l ++ [x]) ls

-- Remove padding
unpad :: [[a]] -> [[a]]
unpad ls = map (tail . init) . tail . init $ ls
